...
def test_with_saved_model(model_path='rf_model.pkl', csv_path='Scenario-A-merged_5s_adversarial.csv'):
...
def main():
    ...
    for p_type in perturb_types:
        for lvl in levels:
            print(f'测试 {p_type} @ level={lvl:.2f}')
            generate_perturbed_data(p_type, lvl)
            acc = test_with_saved_model()
    ...
    df_results.to_csv('rf_perturbation_results.csv', index=False)
    print("\n所有结果已保存至 rf_perturbation_results.csv")
